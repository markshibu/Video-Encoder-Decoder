...
class Frame:
    ...
    #Only for test, not working
    def compare_block(self, m,n,QF):
        tmp = self.getBlock(m,n)
        #print(tmp.shape)
        tmp1 = encode.encode_block(tmp,QF)
        tmp2 = self.decode_block(tmp1,QF)
        ...
        plt.show()
    #Only for test, not working
    def compare_pics(self, QF):
        blocks = []
        for m in range(self.v_mblocks):
            for n in range(self.h_mblocks):
                block = self.getBlock(m,n)
                encoded_block = self.encode_block(block,QF)
                decoded_block = self.decode_block(encoded_block,QF)
        ...
        for m in range(self.v_mblocks):
            ...
            for i in range(16):
                for n in range(self.h_mblocks):
                    block = blocks[m*self.h_mblocks+n]
                    rst+=list(block[i])
            #print(len(rst))
            f+=rst
        #print(len(f))
        f = np.array(f).reshape(self.v_mblocks*16,self.h_mblocks*16,3)
